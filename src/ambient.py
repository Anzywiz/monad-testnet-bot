        ...
        for retry in range(ATTEMPTS):
            try:
                ...
                if swap_type == "collect":
                    ...
                    if token_in == "native":
                        # Use a very small random amount for native token swaps
                        # Generate a random value between 0.00001 and 0.0001
                        amount_token = random.uniform(0.00001, 0.0001)
                        amount_wei = self.convert_to_wei(amount_token, "native")
                    else:
                        if token_in.lower() == "seth":
                            leave_amount = random.uniform(0.00001, 0.0001)
                            balance -= leave_amount
                    ...
                    print_step('swap', f"Swapping {amount_token:.6f} {token_in.upper()} to {token_out.upper()}...")
